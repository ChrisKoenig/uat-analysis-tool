...
#   az acr build -r acrgcsdevgg4a6y -f containers/field-ui.Dockerfile -t gcs/field-ui:latest .
#
...
COPY field-portal/ui/package.json field-portal/ui/package-lock.json ./
RUN npm ci --production=false

COPY field-portal/ui/ ./
...
COPY containers/nginx-field.conf.template /etc/nginx/templates/default.conf.template
