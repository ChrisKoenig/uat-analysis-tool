...
**STATUS** (Mar 11, 2026): System is fully operational locally, deployed to Azure Container Apps (dev), AND deployed to **Azure App Service (pre-prod)**. Pre-prod: 4 App Services in `rg-nonprod-aitriage` — **BOTH Triage and Field Portal deployed and healthy**. Cosmos DB with 16 containers (added `apply-snapshots`), OpenAI (`openai-aitriage-nonprod`), Key Vault (`kv-aitriage`). MSAL auth via App Registration `GCS-Triage-NonProd`. Triage UI has 14 pages, **65 API endpoints**. **Mar 11 session**: FR-2060 — Rules dropdown expanded from 2 groups (Analysis, ADO) to 4 groups (Analysis, ServiceTree, Graph, ADO). Added 7 ServiceTree fields + 5 Graph/Identity fields with MS Graph lookup at rule evaluation time. GraphUserInfo expanded with email + alias. FieldCombobox labels updated, RuleForm label simplified. 5 files changed. **Merged to main (`3d0209c`) and deployment packages rebuilt** (triage-api.zip 0.5 MB, triage-ui.zip 0.9 MB). Build script fix (`e1d02f5`): switched from `npm ci` + bare `vite` to `npm install` + `npx vite build` for Windows compatibility.
