...
if ($Target -eq "all" -or $Target -eq "triage-api") {
    ...
    # Copy triage package
    Copy-Item (Join-Path $repoRoot "apps" "triage") (Join-Path $staging "triage") -Recurse
    # Copy api package (shared API modules)
    Copy-Item (Join-Path $repoRoot "api") (Join-Path $staging "api") -Recurse
    # Copy services package (microservice agents)
    Copy-Item (Join-Path $repoRoot "services") (Join-Path $staging "services") -Recurse

    # Copy shared modules package
    Copy-Item (Join-Path $repoRoot "shared") (Join-Path $staging "shared") -Recurse
    ...
    # Copy requirements
    Copy-Item (Join-Path $repoRoot "apps" "triage" "requirements.txt") $staging
...
if ($Target -eq "all" -or $Target -eq "field-api") {
    ...
    Copy-Item (Join-Path $repoRoot "apps" "field-portal" "api") (Join-Path $staging "api") -Recurse
    ...
    Copy-Item (Join-Path $repoRoot "apps" "triage" "__init__.py") (Join-Path $staging "triage" "__init__.py")
    Copy-Item (Join-Path $repoRoot "apps" "triage" "config" "__init__.py") (Join-Path $triageConfigDst "__init__.py")
    Copy-Item (Join-Path $repoRoot "apps" "triage" "config" "cosmos_config.py") (Join-Path $triageConfigDst "cosmos_config.py")
...
if ($Target -eq "all" -or $Target -eq "field-ui") {
    ...
    Push-Location (Join-Path $repoRoot "apps" "field-portal" "ui")
    try {
        $publicDir = "public"
        if (-not (Test-Path $publicDir)) { New-Item $publicDir -ItemType Directory | Out-Null }
